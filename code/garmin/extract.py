...
# Daily Metrics
# ---------------------------------------------------------------------
def extract_daily_stats(api: Garmin, target_date=None) -> Dict[str, Any]:
    """
    ...
    last_monday_iso = get_last_monday(target_date).isoformat()

    try:
        training_status_data = api.get_training_status(date_iso)
    ...
    try:
        hrv = api.get_hrv_data(date_iso).get("hrvSummary", {}).get("lastNightAvg")
    except Exception:
        hrv = None

    try:
        sleep_score = api.get_sleep_data(date_iso).get("dailySleepDTO", {}).get("sleepScores", {}).get("overall", {}).get("value")
    except Exception:
        sleep_score = None

    try:
        rhr = api.get_rhr_day(date_iso).get("allMetrics", {}).get("metricsMap", {}).get("WELLNESS_RESTING_HEART_RATE", [{}])[0].get("value")
    except Exception:
        rhr = None
...
# Today's Run
# ---------------------------------------------------------------------
def extract_today_run_stats(api: Garmin, target_date=None) -> Dict[str, Any]:
    """
...
# Four Week Averages
# ---------------------------------------------------------------------
def extract_last_four_weeks_stats(api: Garmin, target_date=None) -> Dict[str, Any]:
    """
    ...
    """
    target_date = resolve_date(target_date)
    start_date = get_monday_four_weeks_ago(target_date)
    end_date = get_last_monday(target_date) - timedelta(days=1)
...
# Recency Metrics
# ---------------------------------------------------------------------
def extract_since_last_activity_stats(api: Garmin, target_date=None) -> Dict[str, Any]:
    """
    ...
    """
    target_date = resolve_date(target_date)
    last_monday_four_weeks_ago = get_monday_four_weeks_ago(target_date)
    ...
    if runs:
        ...
        days_since_last_run = (target_date - last_run_date).days
    ...
    if gym_sessions:
        ...
        days_since_last_gym = (target_date - last_gym_date).days
    else:
        days_since_last_gym = None

    quality_session = [activity for activity in activities
                       if activity not in gym_sessions and (activity.get("aerobicTrainingEffect", 0) >= 3 or activity.get("anaerobicTrainingEffect", 0) >= 3)]
    if quality_session:
        ...
        days_since_last_quality_session = (target_date - last_quality_session_date).days
    else:
...
# Location
# ---------------------------------------------------------------------
def extract_location_stats(api: Garmin, target_date=None) -> Dict[str, Any]:
    """
    ...
    """
    target_date = resolve_date(target_date)
    two_weeks_before = get_last_monday(target_date) - timedelta(days=14)

    try:
        activities = api.get_activities_by_date(two_weeks_before.isoformat(), target_date.isoformat(), sortorder="desc")
    except Exception:
        activities = []
...
def combine_garmin_data(api: Garmin, target_date=None) -> Dict[str, Any]:
    """
